$(function () {
    ...
    $('#ratio_container .btn').click(function (event) {
        ...
        alert(dataRatio)
        $image.cropper('destroy').cropper({'aspectRatio': dataRatio,'viewMode':3,'preview': '.img-preview'});
    });
    // 移动函数
    ...
    $('#reset1').click(function (event) {
        event.stopPropagation();
        $image.cropper('crop');
        $image.cropper('destroy').cropper({'preview': '.img-preview','aspectRatio': 4 / 3,'viewMode':3});
    });
    ...
    $('#getCroppedCanvas').click(function (event) {
        event.stopPropagation();
        console.log($image)
        $image.cropper({'preview': '.img-preview','viewMode':3})
    ...
    $('#submit').click(function (event) {
        var imgData = $image.cropper("getCroppedCanvas").toDataURL('image/png');
        var finleName = imgData.split(";")[0].split("/")[1]
        ...
        $.ajax({
            url: path+"/img/addImg?uid="+uid,
            dataType: 'json',
            type: "POST",
            data: {"imageFile": imgData,"finleName":finleName},
            success: function (data) {
                console.log(data)
                $("#warningBox").css("display","block")
                if (data.success=="true") {
                    // alert("上传成功!")
                    $("#warningBox strong").text("上传成功!")
                    $("#headImg").attr("src",data.uImg);
                    // location.reload();
                }else {
                    // alert("上传失败")
                    $("#warningBox strong").text("上传失败")
                }
            },
            error: function (err) {
                console.log(err)
    ...
    var uid = $("#headImg").attr("uid");
    $.ajax({
        url:path+"/img/getImgByUserId",
        type:"POST",
        data:{"userId":uid},
        success:function (data) {
